...
// 资源配额
func (mc *MobileCloud) MobileCloudManageQuotaCheck(kt *kit.Kit, opt *model.QuotaCheckRequestBody, runtimeConfig *config.RuntimeConfig) (*model.QuotaCheckResponse, error) {
	client, err := mc.clientSet.emopClient()
	if err != nil {
		logs.Errorf("new iam client failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
	resp, err := client.ManageQuotaCheck(&model.QuotaCheckRequest[model.QuotaCheckRequestBody]{
		QuotaCheckRequestBody: opt,
	}, runtimeConfig)
